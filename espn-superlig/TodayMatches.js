...
async function run() {
  const seasonStart = new Date("2025-08-08"); // sezon başı
  const seasonEnd = new Date("2026-06-01"); // sezon sonu
  ...
  let csv =
    "Season,League,Week,HomeTeam,AwayTeam,Winner,GoalHome,GoalAway,CornerHome,CornerAway,YellowHome,YellowAway,RedHome,RedAway\n";

  for (const m of allMatches) {
    csv +=
      `${m.Season},${m.League},${m.Week},` +
  ...
  fs.writeFileSync("oynancak_maçlar.txt", csv, "utf-8");

  console.log(`✅ TOPLAM ${allMatches.length} maç (tüm fikstür) yazıldı`);
}
